# CORTEX v8.0 Docker Training Setup
...
From the **CORTEX repository root** (not CORTEX_Training):

```bash
cd C:\Users\Foryoucom\Documents\GitHub\CORTEX
docker-compose -f CORTEX_Training/docker-compose.yml build training-single
```
...
[+] Building 120.5s (18/18) FINISHED
 ...
 => [3/10] COPY CORTEX_Training/requirements.txt .
...
1. Open UE5 project: `CORTEX.uproject`
...
```bash
cd C:\Users\Foryoucom\Documents\GitHub\CORTEX
docker-compose -f CORTEX_Training/docker-compose.yml run --rm test-connection
```
...
```bash
cd C:\Users\Foryoucom\Documents\GitHub\CORTEX
docker-compose -f CORTEX_Training/docker-compose.yml run --rm training-single
```

**Expected output:**
```
============================================================
CORTEX v8.0 Training
============================================================
...
```bash
cd C:\Users\Foryoucom\Documents\GitHub\CORTEX
docker-compose -f CORTEX_Training/docker-compose.yml up tensorboard
```
...
Training results are saved to `CORTEX_Training/training_results/` (mounted volume):

```
CORTEX_Training/training_results/
├── 20260117_123456/              # Timestamp directory
│   ├── cortex_policy_v8.onnx     # Exported ONNX model
...
```bash
docker-compose -f CORTEX_Training/docker-compose.yml up -d training-single
docker-compose -f CORTEX_Training/docker-compose.yml logs -f training-single  # View logs
```

### Stop Training
```bash
docker-compose -f CORTEX_Training/docker-compose.yml down
```

### Clean Up Docker Resources
```bash
# Remove containers
docker-compose -f CORTEX_Training/docker-compose.yml down -v

# Remove image (rebuild required)
docker rmi cortex_training-training-single
...
```bash
docker-compose -f CORTEX_Training/docker-compose.yml run --rm training-single bash
```
...
```bash
cd C:\Users\Foryoucom\Documents\GitHub\CORTEX  # Repository root
docker-compose -f CORTEX_Training/docker-compose.yml build
```
...
3. Enable verbose logging in container:
   ```bash
   docker-compose -f CORTEX_Training/docker-compose.yml run --rm \
...
2. Build multi-worker image:
   ```bash
   docker-compose -f CORTEX_Training/docker-compose.yml build training-multi
   ```
3. Run multi-worker training:
   ```bash
   docker-compose -f CORTEX_Training/docker-compose.yml run --rm training-multi
   ```
...
1. **Export ONNX model:** Automatically saved to `training_results/<timestamp>/cortex_policy_v8.onnx`
2. **Import to UE5:** Copy ONNX file to `Content/AI/NNE/cortex_policy_v8.onnx`
...
- ✅ `CORTEX_Training/Dockerfile` - Updated for v8.0
- ✅ `CORTEX_Training/docker-compose.yml` - Existing, compatible
- ✅ `CORTEX_Training/patch_schola_insecure.py` - Created
- ✅ `CORTEX_Training/DOCKER_SETUP.md` - This file
