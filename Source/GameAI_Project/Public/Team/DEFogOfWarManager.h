...
#include "DEFogOfWarManager.generated.h"
...
struct FDEExploredResource
{
	...
	FDEExploredResource() = default;

	FDEExploredResource(AActor* InActor, FVector InLocation, bool bInAvailable, float InTime)
		: ActorRef(InActor)
		, Location(InLocation)
 ...
 * 2. DEMatchManager references this manager
 * 3. DECharacter calls UpdateVision() every Tick
 * 4. AI queries enemy/resource positions through DEMatchManager → DEFogOfWarManager
 */
UCLASS()
class GAMEAI_PROJECT_API ADEFogOfWarManager : public AActor
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "FogOfWar")
	TArray<FDEExploredResource> GetKnownResources(int32 TeamID) const;
	 ...
	 * Called every tick by DECharacter
...
protected:
	...
	TArray<FDEExploredResource>& GetResourceList(int32 TeamID);
	const TArray<FDEExploredResource>& GetResourceList(int32 TeamID) const;
...
protected:
	...
	UPROPERTY()
	TArray<FDEExploredResource> RedTeamKnownResources;

	UPROPERTY()
	TArray<FDEExploredResource> BlueTeamKnownResources;
