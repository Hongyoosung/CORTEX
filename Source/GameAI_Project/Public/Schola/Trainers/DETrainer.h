...
public:
    ...
    // NOTE: Reward shaping parameters have moved to UDERewardCalculator (on DECharacter).
    // Edit them in the RewardCalculator component of the DECharacter Blueprint.
...
protected:
    ...
    /** DEMatchManager reference — cached for O(1) ally/enemy list access */
    UPROPERTY()
    ADEMatchManager* CachedMatchManager;

    /** DEScholaEnvironment that owns this trainer's arena — cached for match state queries */
    UPROPERTY()
