 ...
 *
 * v6.0 OBSERVATION WITH Mission CONTEXT (68 features total):
 ...
 * - Mission Context (4): type(1), distance(1), direction(2) - NEW in v6.0
 *
 * v6.0 CHANGES (from v5.0):
 * - Added: Mission context (4) - informs RL about MCTS-assigned Mission
 * - Removed: Strategy index (1) - strategy now selected by single-head RL policy
 * - Total: 64 → 68 features
 *
 * Note: Strategy selection is now handled by RL policy based on observation + Mission context.
 */
