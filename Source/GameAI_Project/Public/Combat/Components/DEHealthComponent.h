 ...
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnDamageTaken, const FDEDamageEventData&, DamageEvent, float, CurrentHealth);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnDamageDealt, AActor*, Victim, float, DamageAmount);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnDeath, const FDEDeathEventData&, DeathEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnHealthChanged, float, CurrentHealth, float, MaxHealth);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnKillConfirmed, AActor*, Victim, float, DamageDealt);

/**
 * UDEHealthComponent - AAA-Level Health Management System
 *
 * Features:
 ...
 * 1. DEHealthComponent broadcasts combat events (OnDamageTaken, OnDamageDealt, OnDeath, OnKillConfirmed)
 ...
 *
 * IMPORTANT: DEHealthComponent should NEVER directly call RewardCalculator to avoid double-counting!
 * The event broadcast pattern ensures proper separation of concerns.
 */
UCLASS(ClassGroup=(Combat), meta=(BlueprintSpawnableComponent))
class GAMEAI_PROJECT_API UDEHealthComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	/** Last damage event */
	UPROPERTY(BlueprintReadOnly, Category = "Combat|State")
	FDEDamageEventData LastDamageEvent;
	...
	UPROPERTY(BlueprintAssignable, Category = "Combat|Events")
	FOnDeath OnDeath_Delegate;

	/** Fired when health changes (damage or heal) */
	UPROPERTY(BlueprintAssignable, Category = "Combat|Events")
	FOnHealthChanged OnHealthChanged_Delegate;
