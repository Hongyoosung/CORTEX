...
// FTeamBatchInput and FTeamBatchOutput are now in TeamWorldModelTypes.h

/**
 * MOC v10.2: Team World Model
 * Centralized world model for Squad Commander's MCTS planning.
 *
 * Key Differences from v10.1 UMocAgentWorldModel:
 * - Input: FTeamState (60-dim) instead of individual FObservation (52-dim)
 * - Action Space: ETacticalPlay (~10 plays) instead of EStrategyType (3 strategies)
...
private:
	...
	/**
	 * Internal preprocessing: Convert FTeamState to tensor format
	 * @param BatchInput Input team states and tactical plays
	 * @return Flattened tensor ready for neural network
	 */
	TArray<float> PreprocessBatch(const FTeamBatchInput& BatchInput);
	 ...
	 */
	FTeamBatchOutput PostprocessBatch(const TArray<float>& RawOutput, int32 BatchSize);

	/**
	 * Parse 60-dim tensor back to FTeamState structure
	 * Reverses the transformation done by FTeamState::ToTensor()
