...
#include "DETeamWorldModelTypes.generated.h"

/**
 * Batch Input for Team-Level World Model
 * Used by centralized MCTS to predict outcomes of multiple tactical plays
 *
 * Used by UDETeamWorldModel for batch inference during MCTS planning.
 */
USTRUCT(BlueprintType)
struct GAMEAI_PROJECT_API FDETeamBatchInput
{
	GENERATED_BODY()
	...
	TArray<FDETeamWorldState> CurrentStates;
 ...
 *
 * Returned by UDETeamWorldModel after batch inference during MCTS planning.
 */
USTRUCT(BlueprintType)
struct GAMEAI_PROJECT_API FDETeamBatchOutput
{
	GENERATED_BODY()
	 ...
	 */
	UPROPERTY(BlueprintReadWrite, Category = "WorldModel")
	TArray<FDETeamWorldState> PredictedStates;
	 ...
	 */
	UPROPERTY(BlueprintReadWrite, Category = "WorldModel")
	TArray<FDECompositeReward> Rewards;
...
/**
 * FDETeamReward - Team-level multi-objective reward structure
 *
 ...
 *
 * NOTE: UDETeamWorldModel uses FDECompositeReward for inference.
 * FDETeamReward is kept for backward compatibility with training data collectors.
 */
USTRUCT(BlueprintType)
struct GAMEAI_PROJECT_API FDETeamReward
{
	GENERATED_BODY()
 ...
 *
 * NOTE: UDETeamWorldModel uses FDETeamBatchOutput for batch inference.
 * FTeamStatePrediction can be used for single predictions or legacy code.
 */
USTRUCT(BlueprintType)
struct GAMEAI_PROJECT_API FDETeamStatePrediction
{
	GENERATED_BODY()
