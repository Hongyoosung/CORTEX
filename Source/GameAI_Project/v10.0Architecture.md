...
## Executive Summary
MOC v10.1 implements a **Five-strategy multi-agent hierarchical AI system** 
...
- Multi-Head RL Policy: Five strategy-specialized heads (Assault, Defend, Support, Scout, Retreat) that output 8-dim tactical weights for EQS scoring equations.
...
│ • Expansion: Generate tactical options {Assault, Defend, Support, Scout, Retreat}          │
...
│ Multi-Head Policy Output: 8-dim EQS Weights                     │
│   • Attack Head:  [EnemyObj↑, Cover↓, Visibility↑, Range↑]     │
│   • Defend Head:  [AllyObj↑, Cover↑, AllyProx↑, Height↑]       │
│ ↓                                                                │
│ EQS Query Execution:                                             │
│   • Generator: Circle (Radius=1500, Samples=48)                 │
│   • Tests: 8 weighted spatial evaluations                       │
 ...
 * Input: 61-dim (State=52, OptionIdx=5, Target=3, Duration=1)
...
struct FRLObservation {
    ...
    EStrategyType CurrentOption;   // {0=Attack, 1=Defend, 2=support, 3=scout, 4=retreat}
...
***```
Five-Head Architecture enables fine-grained tactical specialization: Assault (aggressive engagement), Defend (position holding), Support (ally reinforcement), Scout (information gathering under uncertainty), Retreat (survival optimization)
***
...
self.strategy_heads = nn.ModuleDict({
    ...
    'scout': self.build_action_head(head_input_dim, action_dim),
    'retreat': self.build_action_head(head_input_dim, action_dim)
})
```
...
class MultiHeadRLPolicy(nn.Module):
    ...
    def __init__(self, state_dim=52, option_dim=5, target_dim=3, 
        ...
        # Five strategy-specific action heads
        ...
        self.heads = nn.ModuleDict({
            ...
            'scout':   self._build_action_head(head_input_dim, action_dim),
            'retreat': self._build_action_head(head_input_dim, action_dim)
        })
...
The policy network does not output raw movement vectors. Instead, it generates **8-dimensional tactical weight parameters** that configure UE5's Environment Query System (EQS). This design decouples strategic intent from physical navigation, enabling:
...
#### Weight Parameters (8-dim Output)
...
| **HeightAdvantage** | [-1,1] | Seek elevated positions | +0.5 | +0.7 |
...
2. AllyObjectiveProximity, distance score from friendly base. Higher values ​​indicate greater defensive positioning. "Defend: High (+), Scout: Low (-)"
...
4. EnemyVisibility, whether or not the player has line of sight (Line of Sight) of the enemy. Secure a firing angle. "Assault: High (+), Scout: High (+)"
5. AllyProximity, distance from friendly agents. Formation maintenance objective. "Support: High (+), Scout: Low (-)"
...
8. HeightAdvantage, Z-axis height difference. Gain a vision bonus by occupying high ground. "Scout: High (+), Defend: High (+)"
...
8. Height Difference (terrain advantage calculation)
...
Option:          5-dim (one-hot: [Attack, Defend, Support, Scout, Retreat])
...
def sample_team_composition():
    ...
    compositions = [
        ['Assault', 'Assault', 'Defend', 'Support', 'Scout'],
        ['Assault', 'Defend', 'Defend', 'Support', 'Assault'],
        ['Assault', 'Scout', 'Scout', 'Support', 'Retreat']
    ]
