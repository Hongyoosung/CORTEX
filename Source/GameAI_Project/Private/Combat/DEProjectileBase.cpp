...
void ADEProjectileBase::InitializeProjectile(AActor* InOwner, AActor* InInstigator, float InBaseDamage, const FVector& InDirection)
{
	...
	if (ADECharacter* OwnerChar = Cast<ADECharacter>(InOwner))
	{
...
bool ADEProjectileBase::ValidateHit(AActor* HitActor) const
{
	...
	if (const ADECharacter* HitChar = Cast<ADECharacter>(HitActor))
	{
		...
		{
			const ADECharacter* OwnerChar = Cast<ADECharacter>(OwnerActor);
	...
	{
		// Non-DECharacter target: legacy friendly fire check
		const ADECharacter* OwnerChar = Cast<ADECharacter>(OwnerActor);
