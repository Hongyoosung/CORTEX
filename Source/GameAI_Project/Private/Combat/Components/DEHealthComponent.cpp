...
void UDEHealthComponent::BeginPlay()
{
	...
	UE_LOG(LogTemp, Log, TEXT("🏥 DEHealthComponent initialized: %s (Health: %.1f/%.1f, Armor: %.1f)"),
		*GetOwner()->GetName(), CurrentHealth, MaxHealth, Armor);
}

void UDEHealthComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
float UDEHealthComponent::TakeDamage(float DamageAmount, AActor* Instigator, AActor* DamageCauser,
	const FVector& HitLocation, const FVector& HitNormal)
{
	...
	// Track damage contributors for assist reward calculation
	if (Instigator && Instigator != GetOwner() && ActualDamage > 0.0f)
	{
		DamageContributors.FindOrAdd(Instigator) += ActualDamage;
	}

	// Create damage event
	FDEDamageEventData DamageEvent(Instigator, DamageCauser, ActualDamage, HitLocation, HitNormal);
	LastDamageEvent = DamageEvent;

	// Broadcast events (FollowerAgentComponent handles reward integration)
	OnDamageTaken_Delegate.Broadcast(DamageEvent, CurrentHealth);
	OnHealthChanged_Delegate.Broadcast(CurrentHealth, MaxHealth);
...
void UDEHealthComponent::NotifyDamageDealt(AActor* Victim, float DamageAmount)
{
...
void UDEHealthComponent::NotifyKillConfirmed(AActor* Victim, float InTotalDamageDealt)
{
	if (!Victim)
	{
		return;
	...
	// Broadcast event (FollowerAgentComponent handles reward integration)
	OnKillConfirmed_Delegate.Broadcast(Victim, InTotalDamageDealt);
...
float UDEHealthComponent::Heal(float HealAmount)
{
...
void UDEHealthComponent::SetHealth(float NewHealth)
{
...
void UDEHealthComponent::ResetHealth()
{
	CurrentHealth = MaxHealth;
	Armor = StartingArmor;
	bIsAlive = true;
...
void UDEHealthComponent::Kill(AActor* Killer)
{
	if (!bIsAlive)
	{
		return;
...
float UDEHealthComponent::GetTimeSinceLastDamage() const
{
...
void UDEHealthComponent::ResetCombatStats()
{
...
void UDEHealthComponent::HandleDeath(AActor* Killer, float FinalDamage)
{
	if (bHasDied)
	{
	...
	// Create death event
	FDEDeathEventData DeathEvent(GetOwner(), Killer, FinalDamage, GetWorld()->GetTimeSeconds());
	LastDeathEvent = DeathEvent;

	// Broadcast death event (FollowerAgentComponent handles reward integration)
	OnDeath_Delegate.Broadcast(DeathEvent);

	// Notify killer's DEHealthComponent
	if (Killer && Killer != GetOwner())
	{
		if (UDEHealthComponent* KillerHealth = Killer->FindComponentByClass<UDEHealthComponent>())
		{
...
float UDEHealthComponent::ApplyArmorMitigation(float IncomingDamage)
{
...
void UDEHealthComponent::UpdateHealthRegen(float DeltaTime)
{
...
void UDEHealthComponent::DrawDebugInfo()
{
	if (!GetOwner())
	{
		return;
