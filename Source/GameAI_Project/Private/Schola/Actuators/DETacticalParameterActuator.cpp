...
void UDETacticalParameterActuator::TakeAction(const FBoxPoint& Action)
{
	...
	else
	{
		...
		UE_LOG(LogTemp, Error,
			TEXT("[TacticalActuator] ACTION DROPPED (action #%d) — DECharacter not found! bWeightsDirty will NOT be set."),
			ActionCount);
		UE_LOG(LogTemp, Error,
...
void UDETacticalParameterActuator::InitializeActuator()
{
	...
	else
	{
		...
		UE_LOG(LogTemp, Error, TEXT("[TacticalActuator] ✗ No DECharacter found! Outer chain: %s"), *OuterChain);
	}
...
ADECharacter* UDETacticalParameterActuator::FindDECharacter() const
{
	// Path 1: Direct outer is DECharacter
	if (ADECharacter* Direct = GetTypedOuter<ADECharacter>())
	{
		return Direct;
	}
...
void UDETacticalParameterActuator::SetOwnerCharacter(ADECharacter* InCharacter)
{
	DEAgent = InCharacter;
