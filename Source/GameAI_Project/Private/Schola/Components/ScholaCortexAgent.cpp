#include "ScholaCortexAgent.h"

void UScholaCortexAgent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UScholaCortexAgent::ExecuteOption(const FTacticalOption& Option)
{
    ...
    // 실제 액터(FollowerCharacter)에게 명령 전달
...
void UScholaCortexAgent::ExecuteGymAction(const TArray<int32>& GymActionVector)
{
...
void UScholaCortexAgent::ReloadModels(FString WorldModelPath, FString ValueNetPath)
{
    // Role 3: Model Hot-Load
    WorldModel->LoadFromONNX(WorldModelPath);
    ValueNetwork->LoadFromONNX(ValueNetPath);
    UE_LOG(LogTemp, Log, TEXT("[CORTEX] Models Hot-Loaded via Schola Interface"));
}