...
void ADEPickupBase::BeginPlay()
{
	...
	// Bind overlap event
	CollectionSphere->OnComponentBeginOverlap.AddDynamic(this, &ADEPickupBase::OnCollectionSphereBeginOverlap);
...
bool ADEPickupBase::TryCollect(AActor* Collector)
{
	// Check if available
	if (!bIsAvailable)
	{
...
void ADEPickupBase::ApplyPickupEffect_Implementation(AActor* Collector)
{
	// Base implementation does nothing
	// Override in subclasses
}

bool ADEPickupBase::CanCollect_Implementation(AActor* Collector) const
{
...
void ADEPickupBase::OnCollected(AActor* Collector)
{
	bIsAvailable = false;
	UpdateVisuals();
...
void ADEPickupBase::StartRespawnTimer()
{
	if (RespawnTime <= 0.0f)
	{
		return;
	...
	GetWorld()->GetTimerManager().SetTimer(
		RespawnTimerHandle,
		this,
		&ADEPickupBase::OnRespawnTimerComplete,
		RespawnTime,
		false
	);
}

void ADEPickupBase::OnRespawnTimerComplete()
{
	bIsAvailable = true;
...
float ADEPickupBase::GetRespawnTimeRemaining() const
{
	if (bIsAvailable)
	{
		return 0.0f;
...
void ADEPickupBase::ForceRespawn()
{
...
void ADEPickupBase::Reset()
{
	...
	UE_LOG(LogTemp, Verbose, TEXT("[DEPickupBase] %s reset to available"), *GetName());
}

void ADEPickupBase::UpdateVisuals()
{
...
void ADEPickupBase::OnCollectionSphereBeginOverlap(
	...
	const FHitResult& SweepResult)
{
	...
	if (ADECharacter* DEChar = Cast<ADECharacter>(OtherActor))
	{
		if (DEChar->GetEnvID_Implementation() != EnvID)
		{
			return;
		}
	}
