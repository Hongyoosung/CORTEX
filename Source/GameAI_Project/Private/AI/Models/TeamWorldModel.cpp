...
bool UTeamWorldModel::InitModel(const FString& ModelPath)
{
	...
	if (!FPaths::FileExists(ModelPath))
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Model file not found at %s"), *ModelPath);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Failed to load model file: %s"), *ModelPath);
		return false;
	}

	UE_LOG(LogTemp, Log,
		TEXT("TeamWorldModel: Loaded model data (%d bytes) from %s"),
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Failed to create UNNEModelData object"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Failed to get NNE runtime 'NNERuntimeORTCpu'"));
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Failed to create NNE model from data"));
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Failed to create model instance"));
		return true;
	}
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Invalid tensor descriptors"));
		ModelInstance.Reset();
		return true;
	}
	...
	// 텐서 로그
	UE_LOG(LogTemp, Log,
		TEXT("TeamWorldModel: Input tensors: %d"), InputDescs.Num());
	...
	UE_LOG(LogTemp, Log,
		TEXT("TeamWorldModel: Output tensors: %d"), OutputDescs.Num());
	...
	UE_LOG(LogTemp, Log,
		TEXT("TeamWorldModel: Performing warm-up inference..."));
	PredictBatch(DummyInput);

	bIsModelLoaded = true;

	UE_LOG(LogTemp, Log,
		TEXT("TeamWorldModel: Successfully initialized (Avg Latency: %.2fms)"),
		AverageLatencyMs);
...
FTeamBatchOutput UTeamWorldModel::PredictBatch(const FTeamBatchInput& BatchInput)
{
	...
	FTeamBatchOutput Output;

	const uint32 BatchSize = BatchInput.CurrentStates.Num();
	if (BatchSize == 0 || BatchSize != BatchInput.TacticalPlays.Num())
	{
		UE_LOG(LogTemp, Error, TEXT("TeamWorldModel: Invalid batch input sizes"));
		return Output;
	}

	// 1. Preprocess: Convert FTeamState + ETacticalPlay to tensor
	...
	if (bIsModelLoaded && ModelInstance.IsValid())
	{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("TeamWorldModel: Failed to set input tensor shape"));
			return Output;
		}
		...
		{
			UE_LOG(LogTemp, Error, TEXT("TeamWorldModel: Inference failed with code %d"), RunResult);
	...
	else
	{
		...
		UE_LOG(LogTemp, Verbose, TEXT("TeamWorldModel: Running in stub mode (model not loaded)"));
	}

	// 3. Postprocess: Convert output tensor to FTeamBatchOutput
	...
	UE_LOG(LogTemp, Verbose, TEXT("TeamWorldModel: Predicted batch of %d samples in %.2fms"),
		BatchSize, LatencyMs);
	...
	float& OutConfidence)
{
	...
	FTeamBatchOutput BatchOutput = PredictBatch(BatchInput);
...
TArray<float> UTeamWorldModel::PreprocessBatch(const FTeamBatchInput& BatchInput)
{
	...
	constexpr int32 StateDim = 60; // FTeamState::ToTensor() output
	...
	{
		// 1. Convert FTeamState to tensor (60-dim)
		...
		{
			UE_LOG(LogTemp, Error,
				TEXT("TeamWorldModel: Invalid state tensor size %d, expected %d"),
	...
	if (BatchTensor.Num() != BatchSize * InputDim)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TeamWorldModel: Batch tensor size mismatch: %d vs expected %d"),
...
FTeamBatchOutput UTeamWorldModel::PostprocessBatch(const TArray<float>& RawOutput, int32 BatchSize)
{
	FTeamBatchOutput Output;
	...
	// - Next state: 60-dim (same structure as FTeamState::ToTensor())
	...
	{
		UE_LOG(LogTemp, Error,
			TEXT("TeamWorldModel: Output tensor size mismatch: %d vs expected %d"),
	...
	for (int32 i = 0; i < BatchSize; ++i)
	{
		...
		UE_LOG(LogTemp, VeryVerbose,
			TEXT("TeamWorldModel: Sample %d - WinProb=%.3f, HealthDelta=%.3f, Obj=%.3f, Conf=%.3f"),
...
void UTeamWorldModel::UpdateLatencyStats(float LatencyMs)
{
...
FTeamWorldState UTeamWorldModel::ParseTensorToTeamState(const TArray<float>& Tensor, int32 StartIndex) const
{
	...
	for (int32 i = 0; i < 5; ++i)
	{
		...
		State.FriendlyStrategies[i] = static_cast<EStrategyType>(FMath::Clamp(StrategyValue, 0, 2));
	}
