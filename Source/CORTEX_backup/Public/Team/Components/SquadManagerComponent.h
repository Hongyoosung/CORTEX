...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnFollowerRegistered, AActor*, Follower);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnFollowerUnregistered, AActor*, Follower);

/**
 * Manages follower roster for a team leader.
 * Handles registration, unregistration, and roster queries.
 */
UCLASS(ClassGroup=(CORTEX), meta=(BlueprintSpawnableComponent))
class CORTEX_API USquadManagerComponent : public UActorComponent
{
	GENERATED_BODY()
