"""
Schola Bridge - Python-UE5 Communication Layer for MOC Training
...
Usage:
    from schola_bridge import MocEnvironment

    env = MocEnvironment(host='localhost', port=8888)
    obs = env.reset()
...
class MocEnvironment:
    """
    Python interface to MOC Arena UE5 environment.

    State Space: 52-dim observation (positions, health, visibility)
    Action Space: (OptionType, TargetPos, Duration, EQSWeights)
    Reward: Strategy-conditioned rewards (see MocRewardCalculator)
    """
...
class MocMultiAgentEnvironment:
    """
    ...
    def __init__(
        ...
        self.agents = [
            MocEnvironment(host=host, port=port + i)
...
if __name__ == '__main__':
    ...
    with MocEnvironment(host='localhost', port=8888) as env:
        obs = env.reset()
