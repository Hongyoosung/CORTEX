# DE v10.2: Centralized Commander-Executor Architecture
...
## 1. Executive Summary
**DE v10.2** implements a **Hierarchical Centralized Planning (Commander-Executor) Architecture**. Unlike v10.1's decentralized approach where each agent ran individual MCTS, v10.2 introduces a centralized Squad Commander that performs tactical planning and distributes role assignments.
...
| :--- | :--- | :--- |
| **Game Environment** | `DEGameEnvSpecification.md` | Game Rules, Map (150x150m), Occupation Method, Reward Function, Agent Specification |
...
2. **Layer 2: Executor Agents (Decentralized Execution)** (Mid-Level)
   * Actor: `ADECharacter` (×5) receive role assignments via `SetCommandedStrategy()`
...
3. **Layer 3: EQS Spatial Reasoning** (Low-Level, unchanged from v10.1)
   ...
   * Process: `UDEEQSExecutor` queries 48 samples, 8 weighted tests
...
| `ADECharacter` | `Characters/DECharacter` | Agent: receives strategy commands, executes via EQS |
| `ATeamManager` | `Team/TeamManager` | 5v5 spawning, respawn, squad commander integration |
| `UDEEQSExecutor` | `AI/EQS/DEEQSExecutor` | 7-dim weight EQS query → best tactical location |
| `UTeamDataCollector` | `AI/Training/TeamDataCollector` | (State, Action, NextState, Reward) → CSV for training |
| `DERewardCalculator` | `RL/Rewards/` | Composite reward: WinProb + HealthDelta + ObjectiveScore |
...
* `ADECharacter::SetCommandedStrategy()` — command interface
* `ADECharacter::PerformTacticalAction()` — EQS execution with dynamic weights
