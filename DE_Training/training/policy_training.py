...
class DEv10_2TrainingConfig:
    """Training configuration for DE v10.2.
...
def create_env_config():
    ...
    return {
        "host": DEv10_2TrainingConfig.HOST,
        "base_port": DEv10_2TrainingConfig.PORT,
        "num_envs": DEv10_2TrainingConfig.NUM_UE5_ENVIRONMENTS,
...
if RLLIB_AVAILABLE:
    ...
    class DEv10_2Callbacks(DefaultCallbacks):
...
def create_ppo_config():
    ...
    config = config.env_runners(
        num_env_runners=DEv10_2TrainingConfig.NUM_WORKERS,
        num_envs_per_env_runner=DEv10_2TrainingConfig.NUM_ENVS_PER_WORKER,
    ...
    # Callbacks
    config = config.callbacks(DEv10_2Callbacks)

    # Debugging & Reporting
    config = config.debugging(log_level="WARN")
    config = config.reporting(
        metrics_num_episodes_for_smoothing=10,
        min_sample_timesteps_per_iteration=DEv10_2TrainingConfig.TRAIN_BATCH_SIZE,
    )

    # Training
    config = config.training(
        lr=DEv10_2TrainingConfig.LEARNING_RATE,
        lr_schedule=DEv10_2TrainingConfig.LR_SCHEDULE,
        gamma=DEv10_2TrainingConfig.GAMMA,
        entropy_coeff_schedule=DEv10_2TrainingConfig.ENTROPY_COEFF_SCHEDULE,
        train_batch_size=DEv10_2TrainingConfig.TRAIN_BATCH_SIZE,
        minibatch_size=DEv10_2TrainingConfig.SGD_MINIBATCH_SIZE,
        num_epochs=DEv10_2TrainingConfig.NUM_SGD_ITER,
        lambda_=DEv10_2TrainingConfig.GAE_LAMBDA,
        clip_param=DEv10_2TrainingConfig.CLIP_PARAM,
        vf_clip_param=DEv10_2TrainingConfig.VF_CLIP_PARAM,
        entropy_coeff=DEv10_2TrainingConfig.ENTROPY_COEFF,
        vf_loss_coeff=DEv10_2TrainingConfig.VF_LOSS_COEFF,
        grad_clip=DEv10_2TrainingConfig.GRAD_CLIP,
        use_gae=True,
        use_critic=True,
        use_kl_loss=True,
        kl_coeff=0.2,
    ...
    config.model = {
        ...
        "custom_model_config": {
            ...
            "strategy_dim": 3,
            "eqs_dim": 6,
            "hidden_dims": DEv10_2TrainingConfig.HIDDEN_DIMS,
        },
        "max_seq_len": 20,
    }
...
def register_env():
    ...
    from de_env import DEv10_2MultiAgentEnv

    def env_creator(config):
        return DEv10_2MultiAgentEnv(**config)

    register_env("de_env", env_creator)
...
def export_onnx(algo, output_dir):
    ...
    for strategy_idx, (policy_name, strategy_label) in strategy_policies.items():
        ...
        model_path = os.path.join(output_dir, f"de_policy_{strategy_label}.onnx")
...
def train_with_rllib(args):
    ...
    print("=" * 80)
    print("DE v10.2 - Command-Driven Policy Training")
    print("=" * 80)
    print(f"  Host: {DEv10_2TrainingConfig.HOST}:{DEv10_2TrainingConfig.PORT}")
    print(f"  Workers: {DEv10_2TrainingConfig.NUM_WORKERS}")
    print(f"  UE5 Environments: {DEv10_2TrainingConfig.NUM_UE5_ENVIRONMENTS}")
    ...
    # Initialize Ray
    import tempfile
    ray_temp_dir = os.path.join(tempfile.gettempdir(), "ray_de")
    ...
    output_dir = os.path.join(DEv10_2TrainingConfig.OUTPUT_DIR, f"v10_2_{timestamp}")
...
def run_validation():
    ...
    print("\n" + "="*80)
    print("DE v10.2.1 - Validation Suite (Independent Single-Head Policies)")
    print("="*80 + "\n")
...
def evaluate_checkpoint(checkpoint_path: str):
    ...
    print("\n" + "="*80)
    print("DE v10.2.1 - Checkpoint Evaluation")
    print("="*80 + "\n")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Train DE v10.2 command-driven policy")
    ...
    parser.add_argument("--host", type=str, default=DEv10_2TrainingConfig.HOST,
                       help="UE5 host address")
    parser.add_argument("--port", type=int, default=DEv10_2TrainingConfig.PORT,
                       help="UE5 gRPC port")
    ...
    ╚══════════════════════════════════════════════════════════════════════════╝
        ...
        print("  python policy_training.py --mode rllib --iterations 100")
        ...
        DEv10_2TrainingConfig.HOST = args.host
        DEv10_2TrainingConfig.PORT = args.port
