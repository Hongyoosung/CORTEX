"""
DE v10.2 Synchronous Multi-Agent Environment
...
class DEv10_2Config:
    """Configuration for DE v10.2 environment."""
...
if SCHOLA_AVAILABLE:
    ...
    class DEv10_2MultiAgentEnv(MultiAgentEnv):
        """
        DE v10.2 Synchronous Multi-Agent Environment.
        ...
        def __init__(self, **kwargs):
            ...
            print(f"[DE v10.2] Connecting to {host}:{port}...")
            print(f"[DE v10.2] Multi-environment: {self.num_envs} parallel UE5 envs")
            print(f"[DE v10.2] Architecture: Command-Driven Executor (3 strategies)")
            print(f"[DE v10.2] Obs Space: {DEv10_2Config.OBSERVATION_SIZE}-dim (48 base + 3 composition + 3 strategy one-hot)")
            print(f"[DE v10.2] Action Space: Box([-1, 1]^{DEv10_2Config.NUM_EQS_WEIGHTS}) EQS weights")
            ...
            try:
                ...
                if actual_num_envs != self.num_envs:
                    print(f"[DE v10.2] 🔄 Auto-adjusting: Expected {self.num_envs} environments, discovered {actual_num_envs} from UE5")
                    self.num_envs = actual_num_envs

                total_agents = sum(len(a) for a in self.schola_env.ids)
                print(f"[DE v10.2] ✅ Verified {self.num_envs} environments with {total_agents} total agents")
            ...
            self._obs_space = spaces.Box(
                ...
                shape=(DEv10_2Config.OBSERVATION_SIZE,),
                dtype=np.float32
            )
            # v10.2: 7-dim output (EQS weights in [-1, 1])
            self._action_space = spaces.Box(
                low=-1.0, high=1.0,
                shape=(DEv10_2Config.NUM_EQS_WEIGHTS,),
                dtype=np.float32
            )
            ...
            print(f"[DE v10.2] Initialization complete!")
        ...
        def _update_agent_map(self):
            ...
            print(f"[DE v10.2] Agent map: {len(self._agent_ids)} agents")
        ...
        def _sync_max_episode_steps_from_ue5(self, info_dict):
            ...
            UE5 DETrainer includes 'MaxSteps' in GetInfo().
                ...
                if isinstance(info, dict) and 'MaxSteps' in info:
                    try:
                        ...
                        if ue5_max_steps > 0:
                            ...
                            print(f"[DE v10.2] ✅ Synced max_episode_steps from UE5: {self._max_episode_steps}")
                            return
            ...
            if not self._max_episode_steps_synced:
                print(f"[DE v10.2] ⚠️ Could not sync max_episode_steps from UE5, using default: {self._max_episode_steps}")
        ...
        def reset(self, *, seed=None, options=None):
            ...
            if is_first:
                ...
                if self._force_uniform_strategy:
                    ...
                    print(f"[DE v10.2] Uniform strategy assignment: {dict(sorted(self._agent_strategies.items()))}")
                ...
                if self._force_uniform_strategy:
                    self._assign_uniform_strategies()
                    print(f"[DE v10.2] Episode {self._episode_count} strategy rotation: {dict(sorted(self._agent_strategies.items()))}")
        ...
        def close(self):
            """Clean shutdown."""
            print("[DE v10.2] Closing environment...")
            if hasattr(self.schola_env, 'close'):
                self.schola_env.close()
            print("[DE v10.2] Closed")
